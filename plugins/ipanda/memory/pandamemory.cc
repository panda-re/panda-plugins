...
pm_addr_t PandaPhysicalMemory::get_max_address() { return m_max_address; }

uint8_t PandaPhysicalMemory::get_byte(pm_addr_t addr)
{
    uint8_t retval = 0;
...
{
    if ((uintptr_t)buffer < 0x1000) {
        fprintf(stdout, "[DEBUG: %s] %p %lx %p %lu\n", __func__, pmem, addr, buffer,
                size);
    }
